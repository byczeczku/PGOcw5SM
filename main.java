...
// Tworzenie obiektów różnych typów
        Ksiazka ksiazka1 = new Ksiazka("Wiedźmin", 1990, "Andrzej
                Sapkowski", 320);
                Film film1 = new Film("Zielona Mila", 1999, "Frank Darabont", 189);
// Demonstracja polimorfizmu - przechowywanie różnych typów w
        tablicy typu nadrzędnego
        MediaBiblioteczne[] mediaArray = new MediaBiblioteczne[2];
        mediaArray[0] = ksiazka1; // Obiekt Ksiazka przechowywany jako
        MediaBiblioteczne
        mediaArray[1] = film1; // Obiekt Film przechowywany jako
        MediaBiblioteczne
// Demonstracja polimorfizmu - wywołanie metod na obiektach różnych
                typów
        System.out.println("===== INFORMACJE O WSZYSTKICH MEDIACH =====");
        for (MediaBiblioteczne media : mediaArray) {
// Ta sama nazwa metody, ale wywołana zostanie odpowiednia
            implementacja
...
// Demonstracja operacji wypożyczania i zwracania
        ...
        ksiazka1.wypozycz(); // Próba ponownego wypożyczenia - powinien
        być komunikat, że jest już wypożyczona
        ksiazka1.zwroc(); // Zwrot książki
// Wywołanie metod specyficznych dla poszczególnych typów
        System.out.println("\n===== METODY SPECYFICZNE DLA TYPÓW =====");
        ksiazka1.sprawdzLiczbeStron(); // Metoda specyficzna dla klasy
        Ksiazka
        film1.sprawdzCzasTrwania(); // Metoda specyficzna dla klasy
        Film
// Demonstracja rzutowania do wywołania metod specyficznych poprzez
        referencję typu bazowego
        System.out.println("\n===== RZUTOWANIE TYPÓW =====");
        for (MediaBiblioteczne media : mediaArray) {
